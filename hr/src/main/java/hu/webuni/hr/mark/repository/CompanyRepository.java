...
public interface CompanyRepository extends JpaRepository<Company, Long > { 
	...
	@Query("SELECT AVG(e.salary) as salary, e.positions.name  as pozik FROM Company c INNER JOIN c.companyEmployees e WHERE c.id = :id GROUP BY e.positions.name, e.positions.id ORDER BY AVG(e.salary) DESC")
